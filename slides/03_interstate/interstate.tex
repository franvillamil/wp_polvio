...
\includegraphics[width = \textwidth]{../02_IR/img/outbreak_wwii}
...
% ----------------------------------------------------
\begin{frame}
\frametitle{IR and war}
\centering

\includegraphics[width = 0.8\textwidth]{../02_IR/img/league_of_nations}
...
\begin{itemize}
  ...
  \item<3-> Again, usually applied to wars but can also explain many other things (e.g. trade agreements, EU integration, climate norms)
\end{itemize}
...
% ----------------------------------------------------
\begin{frame}
\frametitle{Perspectives, theories, and evidence}
\centering

\begin{itemize}
  \item These are lens to understand war (and IR)
  \item There are \textit{specific theories} linked to each of them
  \begin{itemize}
    \item usually linked to practical solutions
  \end{itemize}
  \item People use data to test different theories
\end{itemize}
...
% \begin{frame}
% \frametitle{Inter-state wars and empirics}
% \centering
...
\begin{frame}
\frametitle{\textit{Specific} theories on war}
\centering

\begin{itemize}
\item Focus on the \BGyellow{causes} of war:
\begin{itemize}
...
\item<2-> Main realist theories:
  \begin{itemize}
    ...
    \item<2-> \textbf{International hegemony}: no, alliances are actually war-prone -- given international anarchy, we need a Leviathan
    \item<2->[] (be careful when hegemony switches, though)
  \end{itemize}
\end{itemize}
...
\begin{itemize}
  ...
  \begin{itemize}
    \item realist explanations are perhaps more focused on explaining system-wide instability, whereas liberal theories let us `predict' specific conflicts
  \end{itemize}
  \item[]
...
\item<2-> ``Opportunity cost'' hypothesis: if we benefit from trade, and war disrupts trade, the cost of fighting will be higher
  \begin{itemize}
    \item role of \textbf{domestic politics}
...
\includegraphics[width = \textwidth]{img/foreignaffairs}
